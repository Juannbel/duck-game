...
RenderableBox::RenderableBox(uint32_t id, SDL2pp::Renderer& renderer): id(id), position(0, 0) {
    std::shared_ptr<SDL2pp::Texture> texture(TexturesProvider::get_instance(renderer).get_texture("boxes"));
    ...
    animations[LittleDamage] =
            new Animation(*texture, animation_data_provider.get_animation_data("box_little_damage"));
    animations[MediumDamage] =
            new Animation(*texture, animation_data_provider.get_animation_data("box_medium_damage"));
