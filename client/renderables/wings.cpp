...
RenderableWings::RenderableWings(uint8_t duck_id) : duck_id(duck_id), position(0, 0), facing_right(true) {
    std::cout << "Loading wings for duck " << duck_id << std::endl;
...
void RenderableWings::load_animation(const std::string &name) {
    animations[name] = new Animation(
        *TexturesProvider::getTexture("wings"),
        AnimationDataProvider::get_animation_data("wings_" + std::to_string(duck_id) +  "_" + name)
    );
}
