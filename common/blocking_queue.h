 ...
 *
 * If the max size is 0, becomes an unbounded queue.
 * */
...
public:
    ...
    bool try_push(T const& val) {
        ...
        if (q.size() == this->max_size && this->max_size > 0) {
            return false;
        }
...
public:
    ...
    bool try_push(void* const& val) {
        ...
        if (q.size() == this->max_size && max_size != UNBOUNDED) {
            return false;
        }
    ...
    void push(void* const& val) {
        ...
        while (q.size() == this->max_size && max_size != UNBOUNDED) {
            is_not_full.wait(lck);
        }
