...
void BulletManager::update_bullets() {
    ...
    for (auto &[id, bullet] : bullets) {
        ...
        for (int i = 0; i < bullet_updates_per_it; i++) {
            float new_x = hitbox.coords.x + (bullet.speed / bullet_updates_per_it) * std::cos(angle * (std::numbers::pi/180));
            float new_y = hitbox.coords.y - (bullet.speed / bullet_updates_per_it) * std::sin(angle * (std::numbers::pi/180));
