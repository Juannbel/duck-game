...
struct Collision EntityManager::check_near_blocks_collision(struct Rectangle &entity, int16_t new_x, int16_t new_y) {
    int16_t row_index = entity.y/BLOCK_SIZE;
    int16_t i = (row_index < NEAR_CELLS) ?  0 : row_index-NEAR_CELLS;
    int16_t end_i = (row_index+NEAR_CELLS > MAP_HEIGHT_BLOCKS) ? MAP_HEIGHT_BLOCKS : row_index+NEAR_CELLS;
...
struct Collision EntityManager::rectangles_collision(const struct Rectangle &r1, const struct Rectangle &r2){
    struct Collision collision;
    collision.horizontal_collision = false;
    collision.vertical_collision = false;
