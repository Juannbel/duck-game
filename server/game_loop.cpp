...
void GameLoop::load_map() {
    for (int32_t i = 0; i < MAP_HEIGHT_BLOCKS; i++) {
        ...
        blocks[MAP_WIDTH_BLOCKS - 1].rectangle = {MAP_WIDTH_PIXELS - BLOCK_SIZE, i * BLOCK_SIZE,
...
void GameLoop::process_action(const struct action& action) {
...
void update_duck_status(struct Duck& duck, const struct Collision& collision) {
...
struct Collision GameLoop::check_near_blocks_collision(struct Rectangle& duck, int32_t new_x,
                                                       int32_t new_y) {
    int32_t row_index = duck.y / BLOCK_SIZE;
    int32_t i = (row_index < NEAR_CELLS) ? 0 : row_index - NEAR_CELLS;
    int32_t end_i =
            (row_index + NEAR_CELLS > map_info.rows) ? map_info.rows : row_index + NEAR_CELLS;

    int32_t column_index = duck.x / BLOCK_SIZE;
    int32_t j = (column_index < NEAR_CELLS) ? 0 : column_index - NEAR_CELLS;
    int32_t end_j = (column_index + NEAR_CELLS > map_info.columns) ? map_info.columns :
                                                                     column_index + NEAR_CELLS;
        ...
        for (auto& block: vec) {
            ...
            if (aux_collision.horizontal_collision) {
                ...
                bool vertical_collision =
                        rectangles_collision(final_rec, block_rec).vertical_collision;
                if (vertical_collision) {
                    if (new_y > duck.y && new_y + duck.height > block_rec.y &&
                        duck.y < block_rec.y) {
...
struct Collision GameLoop::rectangles_collision(const struct Rectangle& r1,
